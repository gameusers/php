...
function plugin_edit_write()
{
	...
	$wiki_csrf_token = $_COOKIE['wiki_csrf_token'];


	$datetime = new \DateTime();
	$datetime->modify('-1day');
	$datetime_yesterday = $datetime->format('Y-m-d');
	$token_yesterday = md5('gameusers_csrf_token_' . $datetime_yesterday);


	// ----------------------------------------
	//   フォームからトークンが取得できなかった場合、クッキーのトークンが取得できなかった場合はエラー
	// ----------------------------------------

	if ( ! $vars['wiki_csrf_token'] and ! $wiki_csrf_token) return;


	// ----------------------------------------
	//   フォームのトークンとクッキーのトークンが同じでない場合、フォームのトークンが昨日のトークンと違う場合（日付をまたいだケースに対応）はエラー
	// ----------------------------------------

	if (($vars['wiki_csrf_token'] != $wiki_csrf_token) and ($vars['wiki_csrf_token'] != $token_yesterday)) return;


	// ----------------------------------------
	//   言語にja（日本語）が含まれていない場合は処理停止
	// ----------------------------------------
	...
	foreach ($languages_arr as $key => $value) {
		if (preg_match('/^ja/i', $value)) $ja_check = true;
	}

	if ($ja_check === false) return;
