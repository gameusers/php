...
function edit_form($page, $postdata, $digest = FALSE, $b_template = TRUE)
{
	...
	// --------------------------------------------------
	//   オリジナル追加　スパム防止機能　フォームにアクセスするとクッキーが設定される
	// --------------------------------------------------
	
	$datetime = new \DateTime();
	$datetime_today = $datetime->format('Y-m-d');
	$token_today = md5('gameusers_csrf_token_' . $datetime_today);
	
	setcookie('wiki_csrf_token', $token_today, 0, '/');
...
$template
  ...
  <input name="wiki_csrf_token" value="$token_today" type="hidden" />
 </form>
