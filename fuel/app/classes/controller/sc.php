...
class Controller_Sc extends Controller_Base
{
...
// 		// --------------------------------------------------
//
		$tab_top_arr['url'] = URI_BASE . 'sc';
		$tab_top_arr['group'] = 'keyword';
		$tab_top_arr['text'] = 'キーワード検索';
		$tab_top_arr['active'] = ($group === 'keyword') ? true : false;
//
...
// 		}
		...
		{
			array_push($this->data_meta['css_arr'], 'style_ver2.css');
			array_push($this->data_meta['js_arr'], Config::get('js_basic'), 'react/bundle.js');
		}
		else
		{
			array_push($this->data_meta['css_arr'], Config::get('css_basic_min'), 'style.min.css', 'new.min.css');
			array_push($this->data_meta['js_arr'], Config::get('js_basic_min'), 'sc.min.js');
		}
		...
		array_push($temp_arr, $tab_top_arr);
...
// exit();
		...
		$original_js_arr = array(
			...
			'initialStateJson' => $this->initial_state_arr
		);

		$code_original_js = $original_code_basic->javascript_json_encode_non_escape($original_js_arr);
		...
		$view = View::forge('base_ver3_view', $this->data_meta);
		$view->set_safe('lang', $language);
		$view->set_safe('original_js', $code_original_js);
		$view->set_safe('js_cdn_arr', $js_cdn_arr);
