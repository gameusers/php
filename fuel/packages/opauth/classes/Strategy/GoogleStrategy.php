...
class GoogleStrategy extends OpauthStrategy{
	...
	public function request(){
		...
		$params = array(
			...
			'openid.realm' => $this->strategy['openid_realm']
		);
	...
	public function oauth2callback(){
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			...
			//echo ($results->id_token);
			//exit();
			
			if (!empty($results) && !empty($results->access_token)){
				$userinfo = $this->userinfo($results->access_token);
				
				// open_idを得るため　id_token をパースする
				$id_token_arr = explode('.', $results->id_token);
				$id_token_for_open_id = str_replace(array('_','-', '.'), array('+', '/', '='), $id_token_arr[1]);
				$id_token_for_open_id_obj = json_decode(base64_decode($id_token_for_open_id));
				
				$this->auth = array(
					'uid' => $userinfo['id'],
					'open_id' => $id_token_for_open_id_obj->openid_id,
					'info' => array(),
					'credentials' => array(
